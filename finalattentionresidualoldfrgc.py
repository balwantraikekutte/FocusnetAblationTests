...
#trainData = np.load('train9DataAttention.npy')
#trainMask = np.load('train9MaskAttention.npy')
#valData = np.load('val9DataAttention.npy')
#valMask = np.load('val9MaskAttention.npy')
...
callbacks = [
    ...
    ModelCheckpoint("resunet9_attention_checkpointfrgc.h5", monitor='val_loss', verbose=1, save_best_only=True, mode='auto'),
    ReduceLROnPlateau(monitor='val_loss', patience=5, verbose=1, mode='auto', epsilon=0.001, cooldown=0, min_lr=0.5e-7)
            ]

history = model.fit(trainD, trainM, validation_data=(valData, valMask), batch_size=16, epochs=200, verbose=1, shuffle=True, callbacks=callbacks)

model.save('resunet9_atentionfrgc.h5')
...
plt.savefig('lossattentionfrgc.png')
...
plt.savefig('dice_coef_attentionfrgc.png')
...
plt.savefig('jaccard_coef_attentionfrgc.png')

plt.figure(3)
plt.plot(history.history['lr'])
plt.title('Learning Rate')
plt.xlabel('epoch')
plt.savefig('lr_attentionfrgc.png')

...
model.compile(loss='mse', optimizer=SGD(lr=0.0001, momentum=0.9, nesterov=True), metrics=[mean_absolute_error])
...
callbacks = [
    ...
    ModelCheckpoint("val_loss__final_mse_ldmk_checkpoint.h5", monitor='val_loss', verbose=1, save_best_only=True, mode='auto'),
...
model.save('final_ldmk_mse_model.h5')
...
plt.savefig('final_ldmk_loss_mse.png')

plt.figure(1)
plt.plot(history.history['lr'])
plt.title('Learning Rate')
plt.xlabel('epoch')
plt.savefig('final_ldmk_lr_mse.png')
...
plt.savefig('final_ldmk_metric_mse.png')

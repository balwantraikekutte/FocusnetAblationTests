...
block1b = _residual_block(basic_block, filters=64, repetitions=1, is_first_layer=False)(block1concat)
...
block2b = _residual_block(basic_block, filters=128, repetitions=1, is_first_layer=False)(block2concat)
...
block3b = _residual_block(basic_block, filters=256, repetitions=1, is_first_layer=False)(block3concat)
...
block4b = _residual_block(basic_block, filters=512, repetitions=1, is_first_layer=False)(block4concat)
...
callbacks = [
    ...
    ModelCheckpoint("resnet_val_loss_nose_ldmk_checkpoint.h5", monitor='val_loss', verbose=1, save_best_only=True, mode='auto'),
    ReduceLROnPlateau(monitor='val_loss', patience=5, verbose=1, mode='auto', epsilon=0.001, cooldown=0, min_lr=0.5e-7)
            ]

model.fit(trainData, trainLabel, batch_size=16, epochs=200, validation_data=(valData, valLabel), shuffle=True, callbacks=callbacks)

model.save('resnet18_nose_ldmk_model.h5')

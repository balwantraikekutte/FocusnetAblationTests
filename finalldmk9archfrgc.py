...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 27 16:54:35 2018
...
data = np.load('frgctrainreg.npy')
label = np.load('frgclabelreg.npy')

trainData = data[0:3000,:,:,:]
trainLabel = label[0:3000,:]
valData = data[3000:3884,:,:,:]
valLabel = label[3000:3884,:]
...
block1conv1 = Conv2D(64, (1,1), padding = 'same', kernel_initializer = 'he_normal')(block1se)
...
block2conv1 = Conv2D(128, (1,1), padding = 'same', kernel_initializer = 'he_normal')(block2se)
...
block3conv1 = Conv2D(256, (1,1), padding = 'same', kernel_initializer = 'he_normal')(block3se)
...
block4b = _residual_block(basic_block, filters=512, repetitions=1, is_first_layer=False)(block4se)
...
#model.load_weights('resnet_val_loss_bottleneck_ldmk9_checkpoint.h5', by_name=True)

model.load_weights('resunet9_attention_checkpointfrgc.h5', by_name=True)
...
callbacks = [
    ...
    ModelCheckpoint("resnet_val_loss_bottleneck_ldmk9_checkpointfrgc.h5", monitor='val_loss', verbose=1, save_best_only=True, mode='auto'),
    ReduceLROnPlateau(monitor='val_loss', patience=5, verbose=1, mode='auto', epsilon=0.001, cooldown=0, min_lr=0.5e-7)
            ]

history = model.fit(trainData, trainLabel, batch_size=8, epochs=200, validation_data=(valData, valLabel), shuffle=True, callbacks=callbacks)

model.save('resnet_bottleneck_ldmk9_modelfrgc.h5')
...
plt.savefig('final_ldmk_lossfrgc.png')

plt.figure(1)
plt.plot(history.history['lr'])
plt.title('Learning Rate')
plt.xlabel('epoch')
plt.savefig('final_ldmk_lrfrgc.png')
...
plt.savefig('final_ldmk_metricfrgc.png')
